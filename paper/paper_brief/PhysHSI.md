...
这里 actor 的输入就是 $o^{\pi}_t$ ，即 N 帧过去帧观测，每帧包括基座角速度（3），重力投影（3），关节相对默认姿态变化（29），关节速度（29），头手角相对基座的位置（15），上一次 policy 输出的控制指令（29，每个关节相对默认姿态位置），物体相对基座位置（3），物体相对基座姿态（6 ，6d 表示法），物体尺寸（3），最终目标相对基座的位置（3）

critic 输入则还包括基座线速度（通常真机上不准噪声大）
