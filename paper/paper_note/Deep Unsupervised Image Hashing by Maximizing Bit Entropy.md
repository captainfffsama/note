...
> Fig2: bi-half 层.M 是小批量 batch 的大小,K 是特征维数.对于每一位哈希码,即 U 中每一列,我们首先进行排序然后根据其位置是前半段还是后半段,置为 +1 或 -1.而在训练过程中使用 sign 函数来将连续特征分配给最近的二进制码,并减小两者之间的欧式距离.蓝色格子显示来我们方法和直接使用 sign 函数的不同,sign 函数下,这些位置的码应该翻转.
